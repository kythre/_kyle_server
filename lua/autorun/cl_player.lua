...
local isokayforsound = data:ReadString()
	chat.AddText( Color( 255, 255, 255 ), name, " has connected to the server." )
...
local isokayforsound = data:ReadString()
	chat.AddText( teamcolour, name, Color( 255, 255, 255 ), " has spawned in the server. Their ID is: "..steamid )
	if tonumber(isokayforsound) == 1 then
...
local teamcolour = team.GetColor(data:ReadShort())
local isokayforsound = data:ReadString()
	chat.AddText( teamcolour, name, Color( 255, 255, 255 ), " has spawned in the server." )
	if tonumber(isokayforsound) == 1 then
		surface.PlaySound( "garrysmod/save_load1.wav" )
	end
end
...
local steamid = data:ReadString()
local isokayforsound = data:ReadString()
	chat.AddText( teamcolour, name, Color( 255, 255, 255 ), " has left the server. Their ID was: "..steamid )
	if tonumber(isokayforsound) == 1 then
...
local teamcolour = team.GetColor(data:ReadShort())
local isokayforsound = data:ReadString()
	chat.AddText( teamcolour, name, Color( 255, 255, 255 ), " has left the server." )
	if tonumber(isokayforsound) == 1 then
		surface.PlaySound( "garrysmod/save_load2.wav" )
	end
end
