...
local function advjl_Initialize( )
	...
	for i=1,#advjl_groups do
		if not (advjl_groups [ i ].playsound == "") then resource.AddFile ( "garrysmod/" .. advjl_groups [ i ].playsound ) end
		if not (advjl_groups [ i ].leftsound == "") then resource.AddFile ( "garrysmod/" .. advjl_groups [ i ].leftsound ) end
	end

	bEnabled = false -- should reset anyway, but whatever
	timer.Simple (advjl_sleeponstart, function()
		bEnabled = true
	end)
end
...
local function advjl_ShowJoinMessage ( ply, arrid, geo_data )
    ...
    for _, pl in pairs( player.GetAll ( ) ) do
    	if not advjl_sendtoownplayer and pl == ply then continue end
